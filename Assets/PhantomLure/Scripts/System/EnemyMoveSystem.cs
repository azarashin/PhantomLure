    ...
    public partial struct EnemyMoveSystem : ISystem
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            for (int i = 0; i < agentCount; i++)
            {
                ...
                var a = state.EntityManager.GetComponentData<EnemyAgent>(e);
                    ...
                    for (int dx = -1; dx <= 1; dx++)
                    {
                        ...
                        if (map.TryGetFirstValue(nIdx, out int otherI, out var it))
                        {
                            do
                            {
                                ...
                                if (dist > 0.0001f && dist < a.SeparationRadius)
                                {
                ...
                if (neighborCount > 0)
                {
                    sep = math.normalizesafe(sep) * a.SeparationWeight;
                }
                ...
                float speed = a.BaseSpeed * speedScale;
